...
public class RootBeer {
    ...
    private ArrayList<String> detectedChecks = new ArrayList<String>();
    ...
    /**
     * @return an array of all root detection checks which return true.
     * Useful for logging to troubleshoot false positive root detections in production.
     * You should call isRooted() or isRootedWithoutBusyBoxCheck() before calling this otherwise
     * you will get an empty list. Be sure to call clearDetectedChecks() afterwards otherwise
     * duplicates could exist.
     */
    public ArrayList<String> getListOfDetectedChecks() {
        return detectedChecks;
    }

    public void clearDetectedChecks() {
    ...
    public boolean detectTestKeys() {
        ...
        boolean signedWithTestKeys = buildTags != null && buildTags.contains("test-keys");

        if (signedWithTestKeys) {
            detectedChecks.add("Kernel was signed with a test-key");
        }

        return signedWithTestKeys;
    }
    ...
    public boolean checkForBinary(String filename) {
        ...
        for (String path : pathsArray) {
            ...
            if (fileExists) {
                String logMessage = completePath + " binary detected!";
                QLog.v(logMessage);
                detectedChecks.add(logMessage);
                result = true;
            }
        }
    ...
    private boolean isAnyPackageFromListInstalled(List<String> packages){
        ...
        for (String packageName : packages) {
            try {
                ...
                String logMessage = packageName + " ROOT management app detected!";
                QLog.e(logMessage);
                this.detectedChecks.add(logMessage);
                result = true;
            ...
            for (String key : dangerousProps.keySet()) {
                if (line.contains(key)) {
                    ...
                    if (line.contains(badValue)) {
                        String logMessage = key + " = " + badValue + " detected!";
                        QLog.v(logMessage);
                        detectedChecks.add(logMessage);
                        result = true;
                    }
                ...
                if (mountPoint.equalsIgnoreCase(pathToCheck)) {
                    ...
                    for (String option : mountOptions.split(",")){
                        ...
                        if (option.equalsIgnoreCase("rw")){
                            String logMessage = pathToCheck+" path is mounted with rw permissions! "+line;
                            QLog.v(logMessage);
                            detectedChecks.add(logMessage);
                            result = true;
                            break;
    ...
    public boolean checkSuExists() {
        ...
        try {
            ...
            boolean suExists = in.readLine() != null;

            if (suExists) {
                detectedChecks.add("su exists");
            }
    ...
    public boolean checkForRootNative() {
        ...
        try {
            rootBeerNative.setLogDebugMessages(loggingEnabled);
            boolean rootNativeCheckResult = rootBeerNative.checkForRoot(paths) > 0;

            if (rootNativeCheckResult) {
                detectedChecks.add("checkForRootNative() returned true");
            }

            return rootNativeCheckResult;
        } catch (UnsatisfiedLinkError e) {
            return false;
        }
    }
